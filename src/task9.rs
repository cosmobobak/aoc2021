...
fn basin_size(lines: &[&[u8]], row: usize, col: usize, seen: &mut Vec<(usize, usize)>) -> usize {
    let local_height = lines[row][col];
    if local_height == b'9' || seen.contains(&(row, col)) {
        return 0;
    }
    let h = lines.len() - 1;
    let w = lines[0].len() - 1;
    let mut acc = 1;
    if col > 0 && lines[row][col - 1] > local_height {
        acc += basin_size(lines, row, col - 1, seen);
    }
    if col < w && lines[row][col + 1] > local_height {
        acc += basin_size(lines, row, col + 1, seen);
    }
    if row > 0 && lines[row - 1][col] > local_height {
        acc += basin_size(lines, row - 1, col, seen);
    }
    if row < h && lines[row + 1][col] > local_height {
        acc += basin_size(lines, row + 1, col, seen);
    }
    seen.push((row, col));
    acc
    ...
    for row in 0..lines.len() {
        for col in 0..lines[0].len() {
            ...
            if adjs.into_iter().all(|x| x > v) {
                let b = basin_size(&lines, row, col, &mut Vec::with_capacity(150));
